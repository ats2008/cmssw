...
tree_RegionalReco=TTree("track_RegReco","Track_RegReco")
...
ptRegionalReco_hist = TH1D("pt_RegionalReco","ptRegionalvReco",50,-2.5,2.5)
ptRvR_hist = TH1D("pt_RvR","pt_RvR_hist",50,-2.5,2.5)
ptMissed_hist = TH1D("pt_Miss","pt_Miss_hist",50,-2.5,2.5)
delleta_RegionalReco_hist = TH1D("NDellEta_RegionalReco","NDellEta_RegionalReco",50,-2.5,2.5)
dellphi_RegionalReco_hist = TH1D("NDellPhi_RegionalReco","NDellPhi_RegionalReco",50,-5.,5.)
delleta_RvR_hist = TH1D("NDellEta_RvR","NDellEta_RvR",50,-2.5,2.5)
dellphi_RvR_hist = TH1D("NDellPhi_RvR","NDellPhi_RvR",50,-5.,5.)
...
#    print "Event", i
    ...
    for idx in range(n_tracks):
        ptRegionalReco_hist.Fill(track_RegionalReco[idx].charge()/track_RegionalReco[idx].pt())       
        ...
        if mask[idx]:
            ptRvR_hist.Fill(track_RvR[idx_RvR].charge()/track_RvR[idx_RvR].pt())
            for j in range(nRegions[0]):
            ...
            ptMissed_hist.Fill(track_RegionalReco[idx].charge()/track_RegionalReco[idx].pt())                  
...
ptMissed_hist.Draw()
acanvas.SaveAs("ptMissed.png")
ptRegionalReco_hist.Draw()
acanvas.SaveAs("ptRegionalReco.png")
ptRvR_hist.Draw()
acanvas.SaveAs("ptRvR.png")
